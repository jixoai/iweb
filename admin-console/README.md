...
The image build copies this output to `worker/admin-assets/`, the in-project
native static-asset input configured by `../worker/wrangler.jsonc` (celld v0.2
requires `assets.directory` to remain inside the Wrangler project). Do not
convert it into JavaScript source or keep a second asset authority in `worker/`.
