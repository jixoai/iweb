...
//! 执行命令权威——owner-authorized ExecutionCommand 生成（commandId、expected
...
// ---------------------------------------------------------------------------
// ExecutionCommand（唯一执行命令 wire：双 CAS fence + 快照 digest 绑定）
// ---------------------------------------------------------------------------
...
/// 唯一 runtime binding wire 形态（单版本）：七字段，hostABI 钉
/// iweb-wasmd-abi@1.1.0（argv@2 标记耦合；wire.rs validate_with_host_abi 的对位
/// 语义）。admission 事实形（ABI 1.0.0）经 runtime_binding_v2_from_v1 推导。
...
pub fn validate_runtime_binding_v2(binding: &RuntimeBindingIdentityV2) -> Result<(), AdmissionError> {
    ...
    if binding.host_abi != crate::wasm_host_services::HOST_ABI_LITERAL_V2 {
        return Err(err(EXECUTION_COMMAND_V2_INVALID, "/runtimeBinding/hostABI must be the exact service-enabled ABI literal iweb-wasmd-abi@1.1.0"));
    }
...
/// 唯一执行命令形态（单版本化：原 V2 envelope 字段集）。hostServicePolicyDigest
/// 为必填 String——空串 = 该应用不获得 host services（命令本身仍存在）。
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
pub struct ExecutionCommand {
    ...
    /// 期望的 wasm-control-state-v2 单一 controlRevision CAS 计数器。
    ...
    /// HostServicePolicyV2.policyDigest；空串 = 无 host services。
...
fn validate_policy_digest(value: &str, field: &str) -> Result<(), AdmissionError> {
    if value.is_empty() || command_regexes().sha256_hex.is_match(value) {
        Ok(())
    } else {
        Err(err(EXECUTION_COMMAND_V2_INVALID, format!("{field} must be empty or 64 lower-case hex characters")))
    }
}

pub fn validate_execution_command(command: &ExecutionCommand) -> Result<(), AdmissionError> {
    ...
    validate_policy_digest(&command.host_service_policy_digest, "/hostServicePolicyDigest")?;
...
pub fn execution_command_digest(command: &ExecutionCommand) -> Result<String, AdmissionError> {
...
/// 唯一执行完成回执形态（单版本化：原 V2 envelope 字段集；命令全字段回显 +
/// applicationId/matrixRevision:hostServicePolicyDigest/fenceNonce 增量）。
/// hostServicePolicyDigest 空串 = 无 host services（与命令同款零值）。
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
...
pub fn validate_execution_acknowledgement(ack: &ExecutionAcknowledgement) -> Result<(), AdmissionError> {
    let invalid = |result: Result<(), AdmissionError>| result.map_err(|e| err(EXECUTION_ACKNOWLEDGEMENT_V2_INVALID, e.detail));
    ...
    invalid(validate_execution_identity(&ack.identity))?;
    ...
    invalid(validate_runtime_binding_v2(&ack.runtime_binding))?;
    ...
    validate_policy_digest(&ack.host_service_policy_digest, "/hostServicePolicyDigest")?;
    ...
    invalid(check_config_snapshot_coupling(ack.config_revision, &ack.config_snapshot_ref, &ack.config_values_digest))?;
...
pub fn correlate_acknowledgement(command: &ExecutionCommand, ack: &ExecutionAcknowledgement) -> Result<(), AdmissionError> {
...
/// receipt 的 binding 为 admission 事实形（ABI 1.0.0），命令 binding 为单版本 wire
/// 形（ABI 1.1.0）：以 runtime_binding_v2_from_v1 归一比较。
pub fn correlate_drain_receipt(command: &ExecutionCommand, receipt: &DrainReceiptV1) -> Result<(), AdmissionError> {
    ...
    let receipt_binding_v2 = runtime_binding_v2_from_v1(&receipt.runtime_binding)
        .map_err(|e| err(DRAIN_RECEIPT_INVALID, e.detail))?;
    if receipt_binding_v2 != command.runtime_binding {
...
/// 精确 digest（spec ExecutionAcknowledgement 条款）。journalRevision 相等性不在此
/// 强制——receipt 与 ack 是否同一条 completion 落盘由 supervisor 生产布局决定（接线
/// 批次固定后可再收紧）。
pub fn correlate_drain_receipt_with_acknowledgement(ack: &ExecutionAcknowledgement, receipt: &DrainReceiptV1) -> Result<(), AdmissionError> {
...
pub fn parse_command_query_result(bytes: &[u8]) -> Result<CommandQueryResultV1, AdmissionError> {
    ...
    if let Some(received) = &parsed.received {
        ...
        let digest = execution_command_digest(&received.command)?;
...
pub fn journal_observation_from_query(outbox_command: &ExecutionCommand, query: &CommandQueryResultV1) -> Result<JournalObservation, AdmissionError> {
    ...
    let expected_digest = execution_command_digest(outbox_command)?;
...
/// Kernel 生成一条 owner-authorized 命令所需的全部权威输入（单版本 wire：
/// 不再按 admission 行代际分选——无 policy 的应用 policy 钉传空串，命令本身
/// 仍生成，只是不获得 host services）。
...
pub struct KernelCommandPlanInput<'a> {
    ...
    /// resolved HostServicePolicyV2 的 policyDigest；None = 无 host services
    ///（命令 policy 钉为空串零值）。
    pub host_service_policy_digest: Option<&'a str>,
    /// admission registry 的版本行（versionId/packageDigest/runtimeBinding 权威；
    /// binding 为 admission 事实形——hostABI 恒 1.0.0）。
...
/// pointer（本函数不触碰 active pointer；激活属 2.4 的 route CAS）。admission 无
/// policy 的行同样获得命令（policy 钉空串）；host services 的授予与否由数据面按
/// policy 钉决定，不由命令存在与否决定。
pub fn plan_execution_command(input: &KernelCommandPlanInput<'_>) -> Result<PlannedExecutionCommand, AdmissionError> {
    ...
    let host_service_policy_digest = input.host_service_policy_digest.unwrap_or("");
    validate_policy_digest(host_service_policy_digest, "/hostServicePolicyDigest")?;
    let runtime_binding = runtime_binding_v2_from_v1(&input.version_row.runtime_binding)?;
    let command = ExecutionCommand {
        schema_version: 2,
        command_id: input.command_id.to_string(),
        expected_control_revision: input.control_revision,
    ...
    let command_digest = execution_command_digest(&command)?;
    Ok(PlannedExecutionCommand { command, next_generations, command_digest })
}

/// runtime binding 推导：admission 的事实形 binding（ABI 1.0.0）+ service-enabled
/// ABI 字面量（1.1.0）。除 hostABI 外逐字段保留 admission 事实；推导产物必须过
/// validate_runtime_binding_v2（fail-closed，绝不半推导）。activation 的 correlate
/// 复用同一推导（registry/retention 存 admission 事实形，命令/lease 携带 1.1.0 形）。
...
/// fenceNonce 的 Kernel 派生：对 (applicationId, versionId, P, E) 的域分离 SHA-256
/// 前 16 字节的 32 位小写十六进制渲染（host-issued；确定性保证 outbox 重建/恢复
/// 推导出同一命令字节——outbox 持久化后优先复用已存命令，派生只是首次生成）。
...
pub struct CommandOutboxRecord {
    #[serde(rename = "commandId")]
    pub command_id: String,
    /// 唯一 wire 命令（单版本；无 policy 的行 policy 钉为空串）。
    pub command: ExecutionCommand,
...
impl WasmCommandControlState {
    ...
    /// retiring 记录的 binding 为 admission 事实形（ABI 1.0.0）；命令 binding 为
    /// 单版本 wire 形（ABI 1.1.0），echo 比较按 runtime_binding_v2_from_v1 归一。
    pub fn authorize_drain_with_retirement(
        ...
        validate_execution_command(&command)?;
        parse_rfc3339_utc_millis(&created_at).map_err(|e| err(EXECUTION_COMMAND_INVALID, e.detail))?;
        let record_binding_v2 = runtime_binding_v2_from_v1(&record.runtime_binding)
            .map_err(|e| err(DRAIN_RECEIPT_INVALID, e.detail))?;
        if record.drain_command_id != command.command_id
            ...
            || record_binding_v2 != command.runtime_binding
        {
    ...
    pub fn project_drain_receipt(&mut self, expected_control_revision: u64, receipt: &DrainReceiptV1) -> Result<DrainRetirementProjection, AdmissionError> {
        ...
        // receipt 必须寻址一条已授权的 drain 命令并与其 echo 一致（归一 binding 比较
        // 在 correlate_drain_receipt 内完成——receipt 为 admission 事实形）。
        let outbox = self
...
mod tests {
    ...
    // -- golden 向量（TS exampleExecutionCommandV2 同值镜像；fenceNonce 换为
    //    plan_fence_nonce_v2 的确定性派生值，单版本化后即唯一命令 wire。JCS 字节同步
    //    固定以锁定跨实现编码。 --

    const GOLDEN_PREPARE_DIGEST: &str = "354d5b7b34f44973a3afe1f706c545e479415550a45972d82c9bfd7741d4820d";
    const GOLDEN_START_DIGEST: &str = "ed55c0135dffa73831ae81e6e20bd980845eee0fd63060dd2c40e2588e107eca";
    const VECTOR_VERSION_ID: &str = "a405ef8d2951e580f70c465aabb96a32b9a29526998b3ef920edfff3c1caa532-1";
    const EXAMPLE_POLICY_DIGEST: &str = "b21afb8e8cb4e6482b137ef5749ea2b9c39e4ef35619bfb079d4c83bd75bb665";
    ...
    /// TS exampleExecutionCommandV2 同值镜像（prepare、configRevision:2）。
    fn golden_prepare_command() -> ExecutionCommand {
        ExecutionCommand {
            schema_version: 2,
            ...
            matrix_revision: 2,
            host_service_policy_digest: EXAMPLE_POLICY_DIGEST.into(),
            fence_nonce: plan_fence_nonce_v2("vector", VECTOR_VERSION_ID, 1, 1),
    ...
    /// start 变体（configRevision:0 + 空串 policy 钉 = 无 host services 的命令面）。
    fn golden_start_command() -> ExecutionCommand {
        ExecutionCommand {
            schema_version: 2,
            ...
            fence_nonce: plan_fence_nonce_v2("vector", VECTOR_VERSION_ID, 1, 2),
            capability_record_revision: 5,
            capability_record_hash: "2".repeat(64),
            secret_revision: 3,
            secret_snapshot_ref: "5".repeat(64),
    ...
    fn golden_command_digest_matches_ts_oracle_and_round_trips() {
        ...
        assert!(text.contains(format!(r#""fenceNonce":"{}""#, plan_fence_nonce_v2("vector", VECTOR_VERSION_ID, 1, 1)).as_str()));
        assert!(text.contains(r#""expectedControlRevision":12"#));
        assert!(!text.contains("expectedKernelControlRevision"));
        assert_eq!(execution_command_digest(&command).expect("digest computes"), GOLDEN_PREPARE_DIGEST);
        assert_eq!(oracle_domain_digest_v2(EXECUTION_COMMAND_DIGEST_DOMAIN_V2, &jcs), GOLDEN_PREPARE_DIGEST);
        // 规范 JCS round-trip：精确键集（deny_unknown_fields）保真。
        let parsed: ExecutionCommand = serde_json::from_slice(&jcs).expect("golden jcs parses");
        assert_eq!(parsed, command);
    }

    #[test]
    fn golden_start_command_with_null_config_and_empty_policy_matches_oracle() {
        let command = golden_start_command();
        assert!(validate_execution_command(&command).is_ok());
        let jcs = jcs_bytes(&command).expect("command serializes as JCS");
        let text = String::from_utf8(jcs.clone()).expect("jcs is utf-8");
        // Option<String> 必须序列化为显式 null（键不缺席）；空 policy 钉合法（无 host
        // services 的命令面）。
        assert!(text.contains(r#""configSnapshotRef":null"#));
        assert!(text.contains(r#""configValuesDigest":null"#));
        assert!(text.contains(r#""hostServicePolicyDigest":""#));
        assert_eq!(execution_command_digest(&command).expect("digest computes"), GOLDEN_START_DIGEST);
        assert_eq!(oracle_domain_digest_v2(EXECUTION_COMMAND_DIGEST_DOMAIN_V2, &jcs), GOLDEN_START_DIGEST);
    }

    #[test]
    fn command_validation_pins_the_service_enabled_invariants() {
        let command = golden_prepare_command();
        // schemaVersion 必须恒 2。
        let mut wrong_version = command.clone();
        wrong_version.schema_version = 1;
        assert_eq!(validate_execution_command(&wrong_version).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        // ABI 1.0.0 binding 不是合法 wire。
        let mut v1_binding = command.clone();
        v1_binding.runtime_binding.host_abi = crate::wasm_admission::WASM_HOST_ABI_LITERAL.into();
        assert_eq!(validate_execution_command(&v1_binding).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        // matrixRevision 恒 2。
        let mut wrong_matrix = command.clone();
        wrong_matrix.matrix_revision = 1;
        assert_eq!(validate_execution_command(&wrong_matrix).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        // fenceNonce 文法（非 32 位小写十六进制）。
        let mut bad_nonce = command.clone();
        bad_nonce.fence_nonce = "0F1E2D3C4B5A69788796A5B4C3D2E1F0".into();
        assert_eq!(validate_execution_command(&bad_nonce).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        // policy digest 文法：空串合法；垃圾值非法。
        let mut bad_policy = command.clone();
        bad_policy.host_service_policy_digest = "zz".into();
        assert_eq!(validate_execution_command(&bad_policy).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        let mut empty_policy = command.clone();
        empty_policy.host_service_policy_digest = String::new();
        assert!(validate_execution_command(&empty_policy).is_ok());
        // applicationId 文法。
        let mut bad_app = command.clone();
        bad_app.application_id = "Vector".into();
        assert_eq!(validate_execution_command(&bad_app).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        // config 耦合破坏。
        let mut broken_config = command.clone();
        broken_config.config_values_digest = None;
        assert_eq!(validate_execution_command(&broken_config).unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        ...
        assert!(serde_json::from_value::<ExecutionCommand>(value).is_err());
    }
    ...
    #[test]
    fn plan_execution_command_builds_the_uniform_wire_for_policy_and_policyless_rows() {
        let row = vector_registry_row();
        // service 行（policy 在）：命令携带 policy digest + ABI 1.1.0 binding。
        let planned = plan_execution_command(&KernelCommandPlanInput {
            ...
            host_service_policy_digest: Some(EXAMPLE_POLICY_DIGEST),
            version_row: &row,
            capability_record_revision: 5,
            capability_record_hash: &"2".repeat(64),
            secret_revision: 3,
        ...
        // 与 golden 向量完全一致（同 registry 行、同快照输入、同 revisions）。
        assert_eq!(planned.command, golden_prepare_command());
        assert_eq!(planned.command_digest, GOLDEN_PREPARE_DIGEST);
        ...
        // 无 policy 行（policy None）：命令仍生成，policy 钉为空串零值。
        let policyless = plan_execution_command(&KernelCommandPlanInput {
            command_id: "018f1e2c-3d4b-7a5e-9f01-23456789abcd",
            operation: WasmExecutionOperation::Prepare,
            control_revision: 12,
            journal_head: 4,
        ...
        .expect("a policy-less admission row still gets a command");
        assert_eq!(policyless.command.host_service_policy_digest, "");
        assert!(validate_execution_command(&policyless.command).is_ok());
        ...
        let make_input = |application_id: &'static str, policy: Option<&'static str>| KernelCommandPlanInput {
            command_id: "018f1e2c-3d4b-7a5e-9f01-23456789abcd",
            operation: WasmExecutionOperation::Prepare,
            control_revision: 12,
            journal_head: 4,
            sandbox_id: "sbx-vector",
            application_id,
            host_service_policy_digest: policy,
            version_row: &row,
            capability_record_revision: 5,
            capability_record_hash: &capability_hash,
            secret_revision: 3,
        ...
        let bad_policy = plan_execution_command(&make_input("vector", Some("zz")));
        assert_eq!(bad_policy.unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
        let bad_app = plan_execution_command(&make_input("Vector", Some(EXAMPLE_POLICY_DIGEST)));
        assert_eq!(bad_app.unwrap_err().code, EXECUTION_COMMAND_V2_INVALID);
    }

    fn applied_ack(command: &ExecutionCommand, journal_revision: u64) -> ExecutionAcknowledgement {
    ...
    #[test]
    fn acknowledgement_validates_and_rejects_every_echo_mismatch() {
        let command = golden_prepare_command();
        let ack = applied_ack(&command, 6);
        assert!(validate_execution_acknowledgement(&ack).is_ok());
        assert!(correlate_acknowledgement(&command, &ack).is_ok());
        // echo 字段的任一错配均拒绝。
        ...
        let mut wrong_operation = ack.clone();
        ...
        let mut wrong_binding = ack.clone();
        ...
        assert_eq!(correlate_acknowledgement(&command, &wrong_policy).unwrap_err().code, "POLICY_MISMATCH");
        let mut wrong_nonce = ack.clone();
        wrong_nonce.fence_nonce = "f".repeat(32);
        assert_eq!(correlate_acknowledgement(&command, &wrong_nonce).unwrap_err().code, "IDENTITY_MISMATCH");
        let mut wrong_app = ack.clone();
        wrong_app.application_id = "other".into();
        assert_eq!(correlate_acknowledgement(&command, &wrong_app).unwrap_err().code, "IDENTITY_MISMATCH");
        let mut wrong_secret = ack.clone();
        ...
        let mut wrong_config = ack.clone();
        ...
        let mut wrong_capability = ack.clone();
        ...
        // 结构校验负例：matrixRevision、schemaVersion、applied 带 failureCode。
        let mut wrong_matrix = ack.clone();
        wrong_matrix.matrix_revision = 1;
        assert_eq!(validate_execution_acknowledgement(&wrong_matrix).unwrap_err().code, EXECUTION_ACKNOWLEDGEMENT_V2_INVALID);
        let mut wrong_version = ack.clone();
        wrong_version.schema_version = 1;
        assert_eq!(validate_execution_acknowledgement(&wrong_version).unwrap_err().code, EXECUTION_ACKNOWLEDGEMENT_V2_INVALID);
        let mut bad_ack = ack.clone();
        bad_ack.failure_code = Some("X".into());
        assert_eq!(validate_execution_acknowledgement(&bad_ack).unwrap_err().code, EXECUTION_ACKNOWLEDGEMENT_V2_INVALID);
        // drain 回执耦合：applied drain 必须携带 digest；非 drain 不得携带。
        let mut drain_ack = ack.clone();
    ...
    fn projection_failure_only_replay_advances_and_never_routes_first() {
        ...
        interleaved.expected_control_revision = 1;
    ...
    fn journal_observation_from_query_validates_byte_identical_replay() {
        let command = golden_prepare_command();
        let digest = execution_command_digest(&command).expect("digest");
        ...
        let tampered_received = CommandReceivedV1 {
            command_digest: execution_command_digest(&other).expect("tampered digest computes"),
            command: other,
            ..received.clone()
        };
    ...
    fn golden_drain_receipt_digest_matches_ts_oracle() {
        ...
        let mut hasher = Sha256::new();
        hasher.update(DRAIN_RECEIPT_DIGEST_DOMAIN.as_bytes());
        hasher.update(b"\n");
        hasher.update(&payload);
        assert_eq!(hex::encode(hasher.finalize()), GOLDEN_DRAIN_RECEIPT_DIGEST);
    ...
    /// 投影世界：已授权 drain 命令（rev 1）+ retiring 记录（rev 2）。drain 命令为
    /// 单版本 wire（ABI 1.1.0 binding），retiring 记录存 admission 事实形。
    fn drain_projection_world() -> (WasmCommandControlState, ExecutionCommand) {
        ...
        command.expected_control_revision = 0;
    ...
    /// drain 命令与 retiring 记录在同一 CAS 中绑定（归一 binding echo）。
    #[test]
    fn authorize_drain_with_retirement_echoes_the_uniform_wire() {
        let command = drain_projection_world().1;
        let mut state = WasmCommandControlState::default();
        state
            .authorize_drain_with_retirement(
                0,
                command.clone(),
                "2026-08-26T00:00:10.000Z".into(),
                RetiringExecutionRecord {
                    drain_command_id: command.command_id.clone(),
                    application_id: "vector".into(),
                    execution: command.identity.clone(),
            ...
            .expect("a uniform drain command binds its retiring record");
        assert_eq!(state.control_revision, 1);
        assert_eq!(state.find_outbox(&command.command_id).expect("outbox").command, command);
    }
    ...
    fn drain_stale_world(retiring: RetiringExecutionRecord) -> (WasmCommandControlState, ExecutionCommand) {
        let mut command = golden_start_command();
        command.command_id = "018f1e2c-3d4b-7a5e-9f01-23456789abcd".into();
        command.operation = WasmExecutionOperation::Drain;
        command.expected_control_revision = 0;
    ...
    fn delivery_attempt_bookkeeping_consumes_one_revision_and_is_idempotent_for_terminal_states() {
        ...
        let mut ack = applied_ack(&command, command.expected_journal_revision + 2);
