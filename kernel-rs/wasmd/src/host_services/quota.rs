...
impl QuotaLedger {
    ...
    pub fn reserve(
        ...
        let tx = self.conn.transaction().map_err(QuotaError::from)?;
        let usage = {
        ...
        let projected_total = usage.committed_bytes.saturating_add(outstanding_total).saturating_add(reserved_delta_bytes);
        ...
        let projected_backend = backend_committed.saturating_add(outstanding_backend).saturating_add(reserved_delta_bytes);
        ...
        let expires_at = now_ms + reservation_ttl_ms;
    ...
    pub fn reserve_remaining(
        ...
        let delta = envelope_bytes
            .saturating_sub(usage.committed_bytes)
    ...
    pub fn finalize(&mut self, reservation: &ReservationRecordV2, measured_backend_bytes: u64) -> Result<(), QuotaError> {
        ...
        tx.execute(
            ...
            params![kv_bytes as i64 + sql_bytes as i64, kv_bytes as i64, sql_bytes as i64],
        )
        .map_err(QuotaError::from)?;
        tx.commit().map_err(QuotaError::from)?;
        Ok(())
    ...
    /// 记录一条 mutating 请求的终态响应（帧 result payload 字节）。
    #[allow(clippy::too_many_arguments)]
    pub fn record_replay(
        ...
        self.conn
            .execute(
                "INSERT OR REPLACE INTO host_call_replay
                 (request_id, identity_digest, method, payload_digest, response_payload, outcome, expires_at)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                params![request_id, identity_digest, method, payload_digest, response_payload, outcome, expires_at as i64],
            )
            .map_err(QuotaError::from)?;
        Ok(())
    }

    /// 查询 replay 终态。Ok(None) = 未见该 requestId；Some((payloadDigest, response, expired))。
    #[allow(clippy::type_complexity)]
    pub fn lookup_replay(
        &self,
        request_id: &str,
        now_ms: u64,
    ) -> Result<Option<(String, Vec<u8>, bool)>, QuotaError> {
        let row = self
            .conn
            .query_row(
                "SELECT payload_digest, response_payload, expires_at FROM host_call_replay WHERE request_id = ?1",
                params![request_id],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
        ...
        match row {
            None => Ok(None),
            Some((payload_digest, response_payload, expires_at)) => {
                Ok(Some((payload_digest, response_payload, expires_at.max(0) as u64 <= now_ms)))
            }
        }
    }
}
