...
# increment 模板随本目录入镜像；数据面根 /data/wasm-data 不入镜像层——/data 是运行时
# 卷，镜像内 mkdir 会被挂载遮蔽；首启由 scripts/iweb-entrypoint.sh 创建（0711，
# IWEB_WASM_DATA_ROOT 三方同名 env 对齐 kernel/wasmd/supervisor），per-app 0700 目录与
# 0600 数据文件由 wasmd 宿主服务自建。wasmd 二进制无增量：host_services 已编入 wasmd crate。
