...
import { jsonResponse as json } from "../../../../packages/worker-shared/src/responses.ts";
...
export class CollabBoard {
	...
	ready = false;
	pending: BoardPayload | undefined;
	...
	async currentPayload(): Promise<BoardPayload> {
	...
	broadcast(): void {
		const frame = JSON.stringify({ type: "board", ...(this.pending as BoardPayload | undefined) });
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (url.pathname === "/api/messages" && request.method === "POST") {
			...
			const author = typeof record.author === "string" ? record.author.trim().slice(0, 32) : "";
