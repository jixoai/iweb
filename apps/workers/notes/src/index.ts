...
function notesId(env: { NOTES_CELL: DurableObjectNamespace }): DurableObjectId {
...
export class NotesCell {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (noteMatch && request.method === "DELETE") {
      const notes = ((await this.state.storage.get("notes")) as Note[] | undefined) ?? defaultNotes;
      const next = notes.filter((note: Note) => note.id !== noteMatch![1]);
...
export interface Env {
	NOTES_CELL: DurableObjectNamespace;
}
