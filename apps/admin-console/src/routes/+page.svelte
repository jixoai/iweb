<script lang="ts">
	...
	import type { ApplicationProjection, NodeStatus, NodeStatusWithApplications, OwnerKey, RouteStore, Workspace } from "$lib/iweb/contracts";
	import { ApiSession } from "$lib/iweb/session.svelte";
	import { MonitorSession } from "$lib/monitor/monitor-session.svelte";
	import ApplicationResourceTable from "$lib/monitor/application-resource-table.svelte";
	import RouteManager from "$lib/routes/route-manager.svelte";
	import KeyManager from "$lib/keys/key-manager.svelte";
	import WorkspaceManager from "$lib/workspace/workspace-manager.svelte";

	type View = "overview" | "workspace" | "routes" | "keys" | "recovery";
	...
	const navigation: NavigationItem[] = [
		...
		{ id: "keys", label: "密钥", title: "密钥与审计", description: "为部署 Agent 签发可独立吊销的 owner 密钥；控制面操作按密钥归因审计。", icon: KeyRoundIcon },
	...
	// 完整 /v1/status 载荷（含 celld fleet 投影与看门狗投影）；四卡片区只读其中的
	// 基础域/运行时/工作区/容器内存四键。
	let status = $state<NodeStatusWithApplications | null>(null);
	let routeStore = $state<RouteStore | null>(null);
	let workspace = $state<Workspace | null>(null);
	let applications = $state<ApplicationProjection[] | null>(null);
	...
	function formatMeasured(measured: { available: boolean; value?: number } | null | undefined, formatter: (value: number) => string): string {
		return measured?.available && typeof measured.value === "number" ? formatter(measured.value) : "不可用";
	}

	function formatLimits(limits: { cpuMillis: number | null; memoryBytes: number | null; pidLimit: number | null; storageBytes: number | null } | null | undefined): string {
		if (!limits) return "不可用";
		return `内存 ${formatBytes(limits.memoryBytes)} · PID ${limits.pidLimit ?? "无"} · 存储 ${formatBytes(limits.storageBytes)}`;
	}
	...
	async function refresh(): Promise<void> {
		...
		connectionError = "";
		try {
			const [nextStatus, nextRoutes, nextWorkspace, nextOwnerKey, nextApplications] = await Promise.all([
				session.client().status(),
				session.client().routes(),
				session.client().workspace(),
				session.client().ownerKey(),
				session.client().statusApplications()
			]);
			status = nextStatus;
			routeStore = nextRoutes;
			workspace = nextWorkspace;
			ownerKey = nextOwnerKey;
			applications = nextApplications.applications;
		} catch (error) {
...
{#if !isAuthenticated}
	<main class="scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[color-mix(in_srgb,currentColor,transparent)] grid min-h-svh place-items-center overflow-y-auto bg-muted/30 p-4 sm:p-6">
		<section class="grid w-full max-w-5xl overflow-hidden rounded-2xl bg-background shadow-sm lg:grid-cols-[1.1fr_0.9fr]">
			...
			<div class="flex items-center p-6 sm:p-10"><Card.Root class="w-full border-0 shadow-none"><Card.Header class="px-0"><Card.Title>管理员登录</Card.Title><Card.Description>使用任意有效的 owner 密钥登录（bootstrap 恢复密钥或控制台签发的委托密钥）。</Card.Description></Card.Header><Card.Content class="px-0"><form class="flex flex-col gap-5" onsubmit={(event) => { event.preventDefault(); void login(); }}><Field.FieldGroup><Field.Field data-invalid={Boolean(connectionError)}><Field.FieldLabel for="admin-key">管理员密钥</Field.FieldLabel><Input id="admin-key" type="password" autocomplete="current-password" placeholder="输入管理员密钥" bind:value={adminKeyDraft} aria-invalid={Boolean(connectionError)} autofocus /><Field.FieldDescription>任意有效 owner key（bootstrap 或委托密钥）均可登录；密钥只保存在当前标签页，不会上传或写入长期存储。</Field.FieldDescription>{#if connectionError}<Field.FieldError>{connectionError}</Field.FieldError>{/if}</Field.Field></Field.FieldGroup><Button type="submit" disabled={loading}>{#if loading}<Spinner data-icon="inline-start" />{:else}<KeyRoundIcon data-icon="inline-start" />{/if}登录管理控制台</Button></form><Alert.Root class="mt-6"><ShieldCheckIcon /><Alert.Title>恢复入口仍独立存在</Alert.Title><Alert.Description><code>api.&lt;base-host&gt;</code> 位于 celld 之外。Admin 被替换或损坏时，管理员仍可借助 owner key 恢复节点。</Alert.Description></Alert.Root></Card.Content></Card.Root></div>
		</section>
	</main>
{:else}
...
{/if}</section>
			...
			{:else if activeView === "keys"}
				<KeyManager client={session.client()} baseHost={status?.baseHost ?? ""} onrefresh={() => void refresh()} onauthfailure={() => logout(false)} />
			{:else if activeView === "routes"}
				<RouteManager apps={workspaceApps} {routes} {status} monitor={monitor.snapshot} {applications} client={session.client()} protocol={externalProtocol} port={externalPort} onrefresh={() => void refresh()} />
			{:else}
