...
- [ ] 5.2 重建镜像部署 iMac（admin 强制 republish）+ 浏览器契约验证；Codex 代码 review 闭环 —2026-08-26 部署与验证证据：HEAD 精确 arm64 镜像于远程节点构建（本地 daemon registry 路径故障，上下文=干净工作树白名单、无秘密；digest sha256:9885eda9…，旧镜像保留 -prev 供回滚）；构建暴露并修复 HEAD 缺陷：bun.lock v2 需 bun 1.4.0（d24d9e4）；同 env/卷/网络重建容器 + IWEB_RUN_NOTES_CELLD=1；admin 强制 republish 三段证据（同版本幂等跳过 / marker 删除→admin 单独 republish+指针切换 / 二次幂等）；浏览器契约 kernel-browser-contract 1 pass/47 expects + recovery 2 + owner-keys 1；portless 10/10；真实 notes DO 等值验证完成（并档入 4.3）。剩余：Codex whole-change review 闭环
