...
- [ ] 1.2 **wasm manifest 与 generation schema**（部分完成，4091677：NormalizedWasmManifestV1/RuntimeBindingIdentityV1/WasmExecutionIdentityV1/控制态/bootstrap 已入 wasm-execution.ts + 45 测试；余：health v2 与 metrics v1 validator 及其专属向量 → W5）：在 `packages/contracts` 定义 `NormalizedWasmManifestV1`、`RuntimeBindingIdentityV1`、`WasmExecutionIdentityV1`、health v2 和 metrics v1 的 unknown-field-rejecting validator/encoder；证明 wasm manifest 不接受 celld `assets`/`entrypoint` 或执行权字段。向量覆盖初始 `(P=1,E=1)`、no-execution `E=0`、CAS 失败、旧 tuple、以及“celld sequence/generation 不可作为 wasm generation”。
