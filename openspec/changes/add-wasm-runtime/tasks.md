...
- [x] 1.0 **R6/R7/R8 transport/persistence boundary**（2026-08-26 完成：/v1/execution-rpc 与 /v1/rpc 双向互斥含 raw magic 拒收、wasm-control-state-v2 controlRevision CAS + outbox、execution journal 幂等 received→completed、崩溃三态唯一恢复、celld parser 物理隔离测试证明；smuggled command 字段期望码按 spec 强化更新为 CELLD_PROTOCOL_MISMATCH（拒绝只强不弱）；19 测试；SO_PEERCRED/SCM_RIGHTS 留 7.1/7.3 TODO）：实现 `iweb-execution-rpc-v1` 的 `/v1/execution-rpc` HTTP-only envelope、固定 socket 双端 peer auth 与 query/replay；建立独立 `wasm-control-state-v2.json`、`controlRevision` CAS、outbox 和 `wasm-execution-journal-v1`，证明 celld `/v1/rpc` 与 `ControlStateFile` 不被降级解析或原地迁移，并接入 runtime-kind registry/migration receipt。
...
- [x] 1.3 **revision 1 capability matrix 与闭包 scanner**（2026-08-26 完成：4ff0822 矩阵/WIT/错误码/接口级闭包 + 53 测试；二进制 scanner 落 wasm-closure-scanner.ts——wasm-tools 1.258.0 真实 fixture 16 枚（embed/new/adapt/compose/对抗 WAT）、唯一内部解析/re-export 折叠/歧义/类型不一致/cycle/adapter 翻译/预算/声明≠发现 27 测试；变异 240 轮+全前缀截断不崩溃；缺口如实：真实 preview1 reactor 替身、二进制层 cycle 不可构造）：把 `wasi:http/proxy@0.2.8` 的精确 `{package,interface,direction}`/host ABI、`iweb:config@1.0.0` 与 `iweb:secrets@1.0.0` WIT 定义和错误码表落入 contracts。两个 iweb world 必须编译为 component `import store`，不得出现 `export store`。实现由 Component Model type identity 驱动的嵌套 component/core module/re-export/adapter traversal；测试唯一内部解析、歧义、未解析、cycle、adapter host import、预算超限、声明集合不等于发现集合。
...
- [x] 1.5 **Kernel-owned admission transaction**（2026-08-26 完成：admission journal 状态机（staging→visible 全链、崩溃重放、残行/中部损坏 fail-closed）、commit token、AdmissionProofV1、AdmittedVisible 三段谓词、幂等 materializer receipt、并发同 proof 去重、显式 sequence 分配、registry 投影与 celld 物理隔离；golden 与 TS 字节一致（a405ef… 向量、proof JCS 1375 字节、token 双向量）；cargo 81 测试 + clippy deny-all 干净）：实现 admission journal、opaque commit token hash、含完整 `versionId`/`sequence`/`normalizedPolicy` 的 `AdmissionProofV1`、到现行 `VersionRecord`/active pointer 的 projection、`AdmittedVisible` predicate 和 idempotent materializer receipt。逐点故障测试：staging 失败、部分 blob、marker 无 DB、DB 事务失败、DB 成功但 handoff 未发/ack 丢失、ack 后 final DB flip 失败、lease 过期、请求响应丢失、并发同 proof；验证 restart replay 或 GC 的唯一结果。
...
- [x] 2.3 **secrets rotation 线性化**（2026-08-26 完成：固定锁序纯函数 + 五点崩溃注入矩阵唯一收敛 + 连续 rotation reducer 只留最大 revision + journal 无值字节扫描证明 + active snapshot 仅激活后替换）：实现 app mutation fence → Kernel secret-store revision commit/outbox → supervisor sandbox journal fence 的固定锁序；journal 只存 revision/opaque reference。覆盖每个 crash 点、ack 丢失和连续两次 rotation，验证最终只接受最大 revision 的 candidate，active snapshot 保留到下一次激活。
- [x] 2.4 **通用激活 gate**（2026-08-26 完成：route CAS 单点判定（conflict→candidate→revoked→lease 校验→nonce→过期复查）、now==expiresAt 相等即过期、nonce 先于过期且同 digest 幂等返回原始事件、rollback 同 gate + 七字段保留记录、指针已翻无事件的精确重建恢复；lease digest 跨语言 golden 858b284b…cf6e）：按 `application-sandbox` delta，在 Kernel route CAS 事务内再次检查 readiness lease 未过期；rollback event 持久化目标 runtime binding identity。覆盖 lease 恰在 CAS 中过期、candidate killed、route CAS conflict 与旧 active 保留。
...
- [x] 7.2 **per-kind registry/migration wire + crash vector**（2026-08-26 完成：WasmKernelRouteRegistryV1 + 全局 kind-claim CAS（终身绑定/双向冲突/split-brain fail-closed）、KindClaimBootstrapV1（celld 推导/崩溃幂等/digest 变化重推导/PENDING 门）、MigrationRecordV1/receipt（跨 kind 断言/幂等 replay/quarantine）；四组 binding golden + bootstrap 向量与 TS 对齐；26 模块测试，crate 114+3 全绿，clippy deny-all 干净）：实现 `WasmKernelRouteRegistryV1`、全局 kind-claim CAS、`runtimeKind`/binding/proof refs 和带 source/target application ID 的 `MigrationRecordV1`/receipt；用当前无 `controlRevision` 的 celld `ControlStateFile` 生成 `sourceRevision:0` 向量，覆盖 target 写入丢 receipt、source digest/target ID 冲突、目标校验失败 quarantine、幂等 replay、active pointer 不变与 celld parser 隔离；含 `KindClaimBootstrapV1` 向量：bootstrap 前 wasm 准入/发布门关闭（`WASM_KIND_BOOTSTRAP_PENDING`）、中途崩溃幂等重放、celld 控制态 digest 变化强制重推导、同 applicationId 的 wasm 准入冲突、双 bootstrap 记录 split-brain。
