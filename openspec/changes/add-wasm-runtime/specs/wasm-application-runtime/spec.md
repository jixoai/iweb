<!-- 用户原始需求（2026-08-25）：wasm 应用与 celld 应用共享同一沙箱法，但制品、能力与出网契约必须按组件模型重新定义。 -->
<!-- 正交意图：严格制品/digest 与准入提交；Kernel 权威与 wasm execution fence；catalog/node capability/matrix；wasmd/readiness/生命周期/秘密；metrics/发布门 wire。 -->
<!-- 不可调和原因：这些记录在同一次 prepare/activate 中相互校验；拆分会重引入 R4 的字段列表与权威矛盾。 -->
...
现行 celld `packageFilesDigest`、`versionDigest(packageDigest, manifest)`、version `sequence` 和 gateway v1 `generation` 不是本 capability 的算法或 generation。它们继续属于 celld 兼容路径；任何 wasm validator 收到它们而缺少本 spec 所要求字段时必须拒绝，而不是推断转换。
...
```text
{
  ...
  requestId: UUIDv7,
  body: ActivationCommandV1 | ActivationQueryV1 | ActivationReplayV1
}
```

`ActivationCommandV1` is exactly:
...
- **WHEN** the Kernel commits the route pointer, lease consume, and `RouteEventV1` but the client loses the response
...
Every `ExecutionCommandV1` with `operation:"drain"` MUST produce one successful `DrainReceiptV1` or a rejected acknowledgement. `DrainReceiptV1` is exactly:
