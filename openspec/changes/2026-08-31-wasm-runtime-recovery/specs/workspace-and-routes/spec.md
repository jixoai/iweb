...
The Kernel route registry file on the persistent volume is the single write authority for routes and SHALL survive node restarts and container recreation. Node startup MUST NOT overwrite it wholesale: when the registry file exists, the image seed is applied only as a system-route upsert keyed by `hostId` (seed rows with `system:true` may add or update their own host IDs), user routes (`system:false`) are never deleted or overwritten, and a seed row colliding with an existing user route is dropped with an owner-visible log line. Only when the registry file does not exist (first boot) is the seed loaded wholesale. An unparseable seed is skipped with an owner-visible log (never a panic, never a partial apply); a missing registry file with no seed is a fail-closed startup error, because a node without its system routes is not a legal steady state. A user route registered through the control surface remains registered across restarts with no re-registration.
