...
const page = (rows, query) => `<!doctype html>
<html lang="zh-CN">
<head>
	<meta charset="utf-8" />
...
<body>
	...
	<main>
		...
		${rows.map((row) => `<div class="row"><span class="cmd">${escapeHtml(row.command)}</span> — ${escapeHtml(row.description)} <span class="note">（${escapeHtml(row.category)}）</span></div>`).join("")}
	</main>
</body>
</html>`;
...
export default {
	async fetch(request, env) {
		...
		try {
			const { results } = query
				? await env.DB.prepare("SELECT command, description, category FROM cheatsheet WHERE command LIKE ?1 ESCAPE '\\' OR description LIKE ?1 ESCAPE '\\' ORDER BY command LIMIT 50").bind(`%${escapeLike(query)}%`).all()
				: await env.DB.prepare("SELECT command, description, category FROM cheatsheet ORDER BY command LIMIT 50").all();
