...
async function dispatch(name, input, context) {
  switch (name) {
    ...
    case "iweb_application_metrics": {
      ...
      const status = await context.kernelFetch("/v1/status");
      const projection = Array.isArray(status && status.applications)
        ? status.applications.find((app) => app && app.id === applicationId) ?? null
        : null;
    ...
    case "iweb_application_versions": {
      const applicationId = requireApplicationId(input.applicationId);
      const listing = await context.kernelFetch("/v1/applications");
      const projection = Array.isArray(listing && listing.applications)
        ? listing.applications.find((app) => app && app.id === applicationId) ?? null
        : null;
    ...
    case "iweb_application_delete_version": {
      ...
      const projection = Array.isArray(listing && listing.applications)
        ? listing.applications.find((app) => app && app.id === applicationId) ?? null
        : null;
      if (projection && projection.activeVersion && projection.activeVersion.digest === versionId) {
    ...
    default: {
      const action = lifecycleActions[name];
      if (action) {
        const applicationId = requireApplicationId(input.applicationId);
        const versionId = requireVersionId(input.versionId);
        return {
...
export async function dispatchTool(name, args, context) {
  try {
    return await dispatch(name, argsOf(args), context);
  } catch (error) {
